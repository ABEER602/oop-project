        ...
        public void loadData1()
        {
            ...
            if (File.Exists(path1))
            {
                ...
                while ((record = fileVariable.ReadLine()) != null)
                {
                    Bill bills = new Bill
                    {
                        ElectricBills = int.Parse((getField(record, 0))),
                        FoodBills = int.Parse((getField(record, 1))),
                        MoneySpendOnStock = int.Parse((getField(record, 2))),
                        TotalExpenditure = int.Parse((getField(record, 3)))
                    };

                    d.Add_Expenditure(bills);
                }
                fileVariable.Close();
            }
            else
        ...
        public void loadData2()
        {
            ...
            if (File.Exists(path2))
            {
                ...
                while ((record = fileVariable.ReadLine()) != null)
                {
                    ...
                    item.Product_Name = (getField(record, 0));
                    item.Pieces = int.Parse((getField(record, 1)));
                    item.Price = int.Parse((getField(record, 2)));
                    d.Add_Item(item);
