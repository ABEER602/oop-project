    ...
    public class Owner
    {
        ...
        public void loadData1()
        {
            ...
            if (File.Exists(path1))
            {
                ...
                {
                    Bill bills = new Bill();
                    int.TryParse((getField(record, 1)), out bills.ElectricBills);

                    int.TryParse((getField(record, 2)), out bills.FoodBills);

                    if (int.TryParse((getField(record, 3)), out int MoneySpendOnStock))
                    {
                        bills.MoneySpendOnStock = MoneySpendOnStock;
                    }
                    if (int.TryParse((getField(record, 4)), out int TotalExpenditure))
                    {
                        bills.TotalExpenditure = TotalExpenditure;
                    }
                    Add_Expenditure(bills);
                }
                fileVariable.Close();
            }
            else
        ...
        
        {
            ...
            if (File.Exists(path2))
            {
                ...
                {
                    Item item = new Item();
                    item.Product_Name = (getField(record, 1));
                    int.TryParse((getField(record, 2)), out item.Pieces);
                    int.TryParse((getField(record, 3)), out item.Price);
                    Add_Item(item);
